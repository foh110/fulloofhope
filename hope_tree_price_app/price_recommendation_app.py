...
# -------------------------- 5. 网页主界面（仅更新文本说明） --------------------------
def main():
    # 1. 设置背景图片（先做这步，避免覆盖内容）
    # 替换为你的本地图片路径（示例：桌面的“bg.jpg”）
    # background_image_path = r"C:\Users\FOH\Desktop\7e7417373011e4979f714b2f34ea8570.jpg"
    # st.markdown(
    #     f"""
    #     <style>
    #     .stApp {{
    ...
    #         opacity: 0.5;  /* 可选：调整页面透明度（避免背景遮挡内容） */
    #     }}
    #     </style>
    #     """,
    ...
    with st.spinner("正在加载模型..."):
        all_models = load_all_models()  # 直接调用，使用默认路径：./hope_tree_price_app/hope_tree_models
    ...
    # 计算预估毛利（处理输入罐单价≤固定成本的情况）
    if input_price <= fixed_cost:
        gross_profit_rate = 0  # 毛利为负时显示0，并提示
        col6.metric(
            "预估毛利率",
        ...
        gross_profit_rate = (input_price - fixed_cost) / input_price * 100  # 公式：(输入罐单-固定成本)/输入罐单 *100%
        col6.metric(
            "预估毛利率",
            f"{gross_profit_rate:.2f}%",
    ...
    reliability = '高' if (sales_adj_r2 >= 0.3 and returns_adj_r2 >= 0.3) else '中' if (sales_adj_r2 >= 0.1 or returns_adj_r2 >= 0.1) else '低'
    st.write(f"• 店铺/渠道：{selected_sheet}（固定RSP：{fixed_rsp:.2f}元）")
...
# 指定字体文件路径（相对于仓库根目录）
font_path = './hope_tree_price_app/fonts/SimHei.ttf'
